...
// ReadSeeker implements io.ReadSeeker with a restriction on the rate of data
// transfer.
...
// NewReadSeeker restricts all Read operations on r to limit bytes per second.
func NewReadSeeker(r io.ReadSeeker, limit int64) *ReadSeeker {
	return &ReadSeeker{r, New(0, 0), limit, true}
}
...
// SetLimit changes the transfer rate limit to new bytes per second and returns
// the previous setting.
func (r *ReadSeeker) SetLimit(new int64) (old int64) {
	old, r.limit = r.limit, new
	return
}

// SetBlocking changes the blocking behavior and returns the previous setting. A
// Read call on a non-blocking ReadSeeker returns immediately if no additional bytes
// may be read at this time due to the rate limit.
func (r *ReadSeeker) SetBlocking(new bool) (old bool) {
	old, r.block = r.block, new
	return
}

// Close closes the underlying ReadSeeker if it implements the io.Closer interface.
